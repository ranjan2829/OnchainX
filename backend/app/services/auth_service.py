...
from ..scehmas import UserCreate,UserLogin, TokenData
...
class AuthService:
    def __init__(self):
        ...
        self.pwd_context=CryptContext(scehmas=["bcrypt"],deprecated="auto")
    ...
    async def signup(self,user_data:UserCreate,db:Session)->Dict[str,Any]:
        try:
            supabase_response=self.supabase.auth.signup(
    ...
    async def signin(self,user_data:UserLogin,db:Session)->Dict[str,Any]:
        try:
            ...
            db_user=db.query(User).filter(User.supabase_id==supabase_response.user.id).first()
            if not db_user:
                raise HTTPException(
